...
# Suprimir mensajes internos de pupil_apriltags
...
target_tag_size = 0.16             #####   CHANGE AS REQUIRED

# Nombre base para los archivos de salida (se agrega timestamp automaticamente)
timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
EXCEL_FILENAME = f"grinding_data_{timestamp_str}.xlsx"
PRINT_INTERVAL = 0  # segundos entre cada print en terminal

# Codec de video (XVID funciona en Windows y Linux)
...
#####################################################################################################
########### Almacenamiento de datos en memoria (se guarda en Excel al cerrar)
...
def save_pose_and_rpy(current_time, position_xyz, R_world_tag, camera_name):
    ...
    row = {
        "timestamp_s": round(current_time, 4),
        "camera": camera_name,
...
def process_pose_T(matrix, state, point, current_time, camera_name):
    if matrix is None or matrix.shape != (4, 4):
        return point
    rotation_matrix_check = matrix[:3, :3].astype(np.float64)
    if not is_valid_rotation_matrix(rotation_matrix_check):
        return point
    ...
    # Imprimir en terminal cada PRINT_INTERVAL segundos
    now = time.monotonic()
    if now - last_print_time.get(camera_name, 0) >= PRINT_INTERVAL:
        last_print_time[camera_name] = now
        print(
            f"[{camera_name}] "
            f"pos=({row['pos_x']:.3f}, {row['pos_y']:.3f}, {row['pos_z']:.3f}) m | "
            f"rot=({row['roll_deg']}°, {row['pitch_deg']}°, {row['yaw_deg']}°) | "
            f"vel=({row['vel_x']:.3f}, {row['vel_y']:.3f}, {row['vel_z']:.3f}) m/s"
        )
...
def save_to_excel(filename):
    ...
    "vel_x", "vel_y", "vel_z",
    "vel_x_avg5", "vel_x_ema"   # <-- nuevas columnas
    ]
...
#####################################################################################################
########### Funciones de sincronizacion de camaras (sin cambios)
...
#####################################################################################################
########### Inicializar camaras Realsense (color + profundidad)
...
for i, serial in enumerate(serial_slaves):
    ...
    vid_color = cv2.VideoWriter(f"slave_{i+1}_color_{timestamp_str}.avi", VIDEO_CODEC, FRAME_RATE, (448, 252))
    vid_depth = cv2.VideoWriter(f"slave_{i+1}_depth_{timestamp_str}.avi", VIDEO_CODEC, FRAME_RATE, (448, 252))
    print(f"[Video] Grabando → slave_{i+1}_color_{timestamp_str}.avi")
    print(f"[Video] Grabando → slave_{i+1}_depth_{timestamp_str}.avi")
...
print("Corriendo... Presiona ESC para detener y guardar Excel + videos.")
...
try:
    ...
    # Cerrar videos (importante: si no se llama, el archivo queda corrupto)
